...
docker stop $NAME
docker rm $NAME
CMD="docker run -d --name $NAME --restart always -p $PORT:8080 -e SLACK_OAUTH_TOKEN=$SLACK_OAUTH_TOKEN -e SLACK_CHANNEL_ID=$SLACK_CHANNEL_ID -e TEMPLATE=$TEMPLATE -e LLM_TEMPLATE=$LLM_TEMPLATE -e DEBUG=$DEBUG -v $(pwd)/config:/config $NAME"
echo $CMD
$CMD
